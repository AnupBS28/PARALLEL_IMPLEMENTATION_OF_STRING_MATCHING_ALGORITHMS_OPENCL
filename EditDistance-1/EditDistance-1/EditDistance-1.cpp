...
int main() {
    ...
    //clError |= clSetKernelArg(edit_distance_algo, 3, sizeof(cl_mem), (void*)&m_row);
    //clError |= clSetKernelArg(edit_distance_algo, 4, sizeof(cl_mem), (void*)&m_column);
    //clError |= clSetKernelArg(edit_distance_algo, 5, sizeof(cl_mem), (void*)&m_max_row);
    //clError |= clSetKernelArg(edit_distance_algo, 6, sizeof(cl_mem), (void*)&m_max_column);
    ////clError |= clSetKernelArg(edit_distance_algo, 7, sizeof(cl_mem), (void*)&m_test);
    //clError |= clSetKernelArg(edit_distance_algo, 7, sizeof(cl_mem), (void*)&m_indicate);
    //clError |= clSetKernelArg(edit_distance_algo, 8, sizeof(cl_mem), (void*)&m_size);
    if (clError < 0) {
        printf("op %d", clError);
        exit(1);
    }
   ...
   /* for (int h = 2;h < max_column + max_row - 1;h++)
    {
        ...
        for (int gid = 0;gid < globalWorkSize[0];gid++)
        {
            int r;
            r = (row + gid);
            int c;
            c = column - gid;
            int current_index;
            current_index = r * max_column + c;
            int diagonal;
            diagonal = (r - 1) * max_column + c - 1;
            // if(gid==1)
            //{
            ...
            //}
            if (b[r - 1] == a[c - 1]) {
                input_array[r][c] = input_array[r - 1][c - 1];
                ...
                right = diagonal + 1;
                int mi;
                mi = input_array[r - 1][c - 1];
                if (mi > input_array[r][c - 1])
                    mi = input_array[r][c - 1];
                if (mi > input_array[r - 1][c])
                    mi = input_array[r - 1][c];
                input_array[r][c] = mi + 1;
            }
        }
    ...
    for (int h = 2;h < max_column + max_row - 1;h++)
    {
        ...
        clEnqueueWriteBuffer(queue, m_string1, CL_FALSE, 0, str1len * sizeof(char), a, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_string2, CL_FALSE, 0, str2len * sizeof(char), b, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_row, CL_FALSE, 0, 1 * sizeof(cl_uint), &row, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_column, CL_FALSE, 0, 1 * sizeof(cl_uint), &column, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_max_row, CL_FALSE, 0, 1 * sizeof(cl_uint), &max_row, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_max_column, CL_FALSE, 0, 1 * sizeof(cl_uint), &max_column, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_indicate, CL_FALSE, 0, 1 * sizeof(cl_uint), &indicate, 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, m_size, CL_FALSE, 0, 1 * sizeof(cl_uint), &wr, 0, NULL, NULL);*/
    ...
    /*for (int h = max_column + max_row - 1 - str2len / 4;h < max_column + max_row - 1;h++)
    {
        row = h - max_column + 1;
        column = max_column - 1;
        for (int gid = 0;gid < max_row - 1 - g;gid++)
        {
            int r;
            r = (row + gid);
            int c;
