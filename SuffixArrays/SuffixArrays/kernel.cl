...
int cmp_constant_global(const __global struct suffix *a,
                        const __global struct suffix *b) {
...
inline void sortLocal(__local struct suffix *a, __local struct suffix *b,
                      char dir) {
  if (cmp_local(a, b) == dir)
    swapLocal(a, b);
}

__kernel void Sort_MergesortStart(const __global struct suffix *inArray,
                                  __global struct suffix *outArray) {
  ...
  for (unsigned int stride = 2; stride <= MAX_LOCAL_SIZE * 2; stride <<= 1) {
    ...
    if (rightBoundary > MAX_LOCAL_SIZE * 2)
      continue;
#pragma unroll
    for (uint i = 0; i < stride; i++) {
      ...
      bool selectLeft =
          left < middle && (right >= rightBoundary || cmp(&leftVal, &rightVal));
...
__kernel void Sort_MergesortGlobalSmall(const __global struct suffix *inArray,
                                        __global struct suffix *outArray,
                                        const uint stride, const uint size) {
...
#pragma unroll
  for (uint i = baseIndex; i < (baseIndex + stride); i++) {
    local_buffer[baseLocalIndex + (int)selectLeft] =
        (selectLeft) ? inArray[left] : inArray[right];
    selectLeft = left < middle && (right == (baseIndex + stride) ||
                                   cmp_local(&local_buffer[baseLocalIndex + 1],
                                             &local_buffer[baseLocalIndex]));

    outArray[i] = (selectLeft) ? local_buffer[baseLocalIndex + 1]
                               : local_buffer[baseLocalIndex];
...
__kernel void Sort_MergesortGlobalBig(const __global struct suffix *inArray,
                                      __global struct suffix *outArray,
                                      const uint stride, const uint size) {
  const uint baseIndex = get_global_id(0) * stride;
  const char dir = 1;
...
#pragma unroll
  for (uint i = baseIndex; i < (baseIndex + stride); i++) {
    selectLeft =
        (left < middle &&
         (right == (baseIndex + stride) ||
          cmp_constant_global(&inArray[left], &inArray[right]))) == dir;
...
__kernel void init(__global struct suffix *suffixes, __global char *txt,
                   __global int *n) {
...
__kernel void rank_to_suffix(__global struct suffix *suffixes,
                             __global int *indices, const uint n,
                             const uint k) {
  int i = get_global_id(0);
  if (i < n) {
    int nextindex = suffixes[i].index + k;
    suffixes[i].rank[1] =
        (nextindex < n) ? suffixes[indices[nextindex]].rank[0] : -1;
  }
}

__kernel void later(__global struct suffix *suffixes, __global int *indices,
                    __global int *n) {
  int i = get_global_id(0);
  if (i < n[0]) {
